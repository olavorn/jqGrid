...
$(function () {
        ...
        QUnit.test("grid expando exists", function(assert) {
            assert.notEqual( (<JQueryJqGridElement>$grid[0]).grid, undefined, "Passed!");
        });
        QUnit.test("p expando exists", function(assert) {
            assert.notEqual( (<JQueryJqGridElement>$grid[0]).p, undefined, "Passed!");
        });
        QUnit.test("rows of grid exist - it's table", function(assert) {
            assert.notEqual( (<JQueryJqGridElement>$grid[0]).rows, undefined, "Passed!");
        });
        QUnit.test("grid has 11 rows", function(assert) {
            assert.equal( (<JQueryJqGridElement>$grid[0]).rows.length, 11, "Passed!");
        });
        QUnit.test("grid has 1 hidden first rows (with class jqgfirstrow)", function(assert) {
            assert.equal( $((<JQueryJqGridElement>$grid[0]).rows[0]).hasClass("jqgfirstrow"), true, "Passed!");
        });
        ...
        QUnit.test("the first row if data have id 11", function(assert) {
            assert.equal( (<JQueryJqGridElement>$grid[0]).rows[1].id, "11", "Passed!");
        });
        QUnit.test("formatted 2007-10-01 is 01-Oct-2007", function(assert) {
            assert.equal($( (<JQueryJqGridElement>$grid[0]).rows[1].cells[1]).text(), "01-Oct-2007", "Passed!");
        });
        QUnit.test("0 formatted as number is 0.00", function(assert) {
            assert.equal($( (<JQueryJqGridElement>$grid[0]).rows[1].cells[2]).text(), "0.00", "Passed!");
        });
        ...
        QUnit.test("test add data by addRowData for the rowid=10", function(assert) {
            ...
            assert.equal((<JQueryJqGridElement>$grid[0]).rows.length, 12, "the grid has 11 data rows");
            assert.equal($((<JQueryJqGridElement>$grid[0]).rows[11].cells[1]).text(), "03-Apr-2015", "2015-04-03 is displayed as 03-Apr-2015");
            assert.equal($((<JQueryJqGridElement>$grid[0]).rows[11]).hasClass("myAltRowClass"), false, "the last row has no myAltRowClass");
            assert.equal($((<JQueryJqGridElement>$grid[0]).rows[10]).hasClass("myAltRowClass"), true, "the previous row has myAltRowClass");
        });
    });
